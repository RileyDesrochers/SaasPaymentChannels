...
function _sol(ar, depth){
    ...
    for(let n=0;n<ar.length;n++){
        let cur = ar[n][1];
        let tmp = allMoves(ar[n][0]) 
        for(let m=0;m<tmp.length;m++){
            //validatedata(tmp[m])
            let ths = cubeToBig(tmp[m]);
            if(typeof (sol[ths]) !== 'undefined'){
                console.log("SOLVED IN ", cur.length + sol[ths].length, " MOVES")
                //console.log(ths)
                //console.log(sol[ths])
                //checkPerm(tmp[m])
                console.log(cur.concat(sol[ths]))
                //printSol(sta[ths][1].concat(sol[ths][1]))
                return;
            }
            //if(cur.length < 4){
            arn.push([tmp[m], cur.concat(m)])//concat(m)
            //}
        }
        /*if(n%10000 === 0)(
    ...
    for(let n=0;n<1000000;n++){
        ...
        for(let m=0;m<tmp.length;m++){
            ...
            if(typeof (sta[ths]) === 'undefined'){
                sta[ths] = cur.concat(m)
                if(typeof (sol[ths]) !== 'undefined'){
                    console.log("SOLVED IN ", sol[ths].length + sta[ths].length, " MOVES")
                    console.log(ths)
                    //console.log(sol[ths])
                    //checkPerm(tmp[m])
                    console.log(sta[ths].concat(sol[ths]))
                    //printSol(sta[ths][1].concat(sol[ths][1]))
                    return;
                }
                if(cur.length < 5){
                    ...
                    _sol([[tmp[m], sta[ths]]], 2)
                    //arn.push([tmp[m], sta[ths]])
                }
                //arra.push(tmp[m])
            }
        }
